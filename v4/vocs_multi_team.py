...
# DRY RUN MODE - set to True to skip Slack posting
...
def load_and_prepare_feedback(csv_path: Path, team_config: Dict) -> tuple[pd.DataFrame, Dict]:
    ...
    # CRITICAL FIX: Calculate metrics for ALL rows BEFORE filtering
    total_rows = len(df)
    
    # CES breakdown for ALL rows
    ces_breakdown_all = {}
    ces_avg_all = 0.0
    negative_count_all = 0
    
    if ces_col in df.columns:
        ces_breakdown_all = df[ces_col].value_counts().to_dict()
        ces_avg_all = df[ces_col].mean()
        negative_count_all = len(df[df[ces_col] <= 2])
    
    print(f"  📊 Total CSV rows: {total_rows}")
    print(f"  📊 CES breakdown (ALL): {ces_breakdown_all}")
    
    # NOW prepare feedback columns
    df = df.copy()
    feedback_parts = []
    ...
    # NOW filter to rows with feedback (for analysis only)
    df_with_text = df[df['_combined_feedback_'] != ""].copy()
    
    rows_with_feedback = len(df_with_text)
    rows_without_feedback = total_rows - rows_with_feedback
    
    print(f"  📊 Rows with comments: {rows_with_feedback}")
    print(f"  📊 Rows score-only: {rows_without_feedback}")
    
    metadata = {
        'total_rows': total_rows,  # ALL ROWS
        'rows_with_text': rows_with_feedback,  # Only for analysis
        'rows_without_text': rows_without_feedback,
        'ces_breakdown_all': ces_breakdown_all,  # CES for ALL
        'ces_avg_all': ces_avg_all,
...
def customize_prompt(template: str, team_config: Dict, metadata: Dict, week_number: str, last_week_data: Optional[Dict]) -> str:
    ...
    ces_breakdown = metadata.get('ces_breakdown_all', {})  # Use ALL rows breakdown
    
    # Calculate WoW trends using ALL rows
    wow_trends = ""
    if last_week_data:
        total_change = metadata['total_rows'] - last_week_data['total_feedback']
        ...
        ces_change = metadata.get('ces_avg_all', 0) - last_week_data.get('ces_avg', 0)
        ces_arrow = "↓" if ces_change < 0 else "↑" if ces_change > 0 else "→"
        
        neg_change = metadata.get('negative_count_all', 0) - last_week_data.get('negative_count', 0)
...
- Total responses: {total_arrow} {abs(total_pct):.0f}% ({last_week_data['total_feedback']} → {metadata['total_rows']})
- Average CES: {ces_arrow} {abs(ces_change):.2f} ({last_week_data.get('ces_avg', 0):.2f} → {metadata.get('ces_avg_all', 0):.2f})
- Negative scores (1-2): {neg_arrow} {abs(neg_pct):.0f}% ({last_week_data.get('negative_count', 0)} → {metadata.get('negative_count_all', 0)})
- Comments received: {metadata['rows_with_text']} ({(metadata['rows_with_text']/metadata['total_rows']*100):.1f}% comment rate)
"""
    else:
        wow_trends = f"\nFIRST WEEK - No previous data for comparison\nComment rate: {metadata['rows_with_text']}/{metadata['total_rows']} ({(metadata['rows_with_text']/metadata['total_rows']*100):.1f}%)\n"
    
    context_header = f"""
IMPORTANT CONTEXT:
- Product Area: {team_config['name']}
- Week: {week_number}
- Total Survey Responses: {metadata['total_rows']}
- Responses with Comments: {metadata['rows_with_text']}
- Responses with Score Only: {metadata['rows_without_text']}
- Data includes: Session IDs, CES scores (1-5), dates, and customer comments

NOTE: You are analyzing ONLY the {metadata['rows_with_text']} responses that included written feedback.
However, the CES breakdown below includes ALL {metadata['total_rows']} responses.
...
"""
    ...
    prompt = context_header + template.format(
        ...
        comments_count=metadata['rows_with_text'],  # Only with feedback
...
def process_team(team_config: Dict, prompt_template: str, week_number: str) -> bool:
    ...
    # Calculate metrics for storage - USE ALL ROWS from metadata
    ces_avg = metadata['ces_avg_all']
    negative_count = metadata['negative_count_all']
    ...
    # Analyze with Claude
    if DRY_RUN:
       print(f"  🤖 DRY RUN - Skipping Claude API call")
       analysis = "[DRY RUN MODE - Claude API call skipped]\n\nThis would contain the AI analysis."
    else:
       print(f"  🤖 Calling Claude API...")
       analysis = analyze_with_claude(prompt, feedback_summary)
       print(f"  ✅ Analysis complete ({len(analysis)} characters)")
    
    # Debug: Check DRY_RUN value
    ...
    if DRY_RUN:
       print(f"  🧪 DRY RUN - Would post to Slack: {channel}")
       print(f"  📝 Message preview (first 300 chars):")
       print(f"  {analysis[:300]}...")
       print(f"  [Slack posting SKIPPED in dry-run mode]")
    else:
    ...
    # Save to database
    if DRY_RUN:
       print(f"  🧪 DRY RUN - Would save to database (SKIPPED)")
    else:
       save_team_summary(team_id, team_name, week_number, metadata['total_rows'], 
